...
from create_csv import  write_own_csv
...
class EmotionRecognizer:
    def __init__(self,model=None, **kwargs):
        ...
        self.db = kwargs.get("db", True)
        self.path = kwargs.get("path","data/db/*.wav")
        ...
        self.db_name = kwargs.get("db_name", "db.csv")
    ...
    def _set_metadata_filenames(self):
        ...
        if self.db:
            train_desc_files.append(f"train_{self.db_name}")
            test_desc_files.append(f"test_{self.db_name}")
    ...
    def _verify_emotions(self):
        ...
        for emotion in self.emotions:
            assert emotion in AVAILABLE_EMOTIONS 
        ...
        for train_csv_file, test_csv_file in zip(self.train_desc_files, self.test_desc_files):
            ...
            if os.path.isfile(train_csv_file) and os.path.isfile(test_csv_file):
                ...
                if not self.override_csv:
                    continue
            if self.db_name in train_csv_file:
                write_own_csv(self.emotions, train_name=train_csv_file, test_name=test_csv_file, verbose=self.verbose, path=self.path)
    ...
    def determine_best_model(self):
        ...
        for estimator, params, cv_score in estimators:
            ...
            detector = EmotionRecognizer(estimator, emotions=self.emotions, 
                                        db=self.db, classification=self.classification,
